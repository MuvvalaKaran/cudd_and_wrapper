  ...
  )
{
  ...
  intStringLength = (int) strlen (intString);
  ...
  switch (mode) {
    ...
    case DDDMP_CNF_MODE_MAXTERM:
      *varNewNPtr = 0;
      break;
    default:
      Dddmp_Warning (1, "Wrong DDDMP Store Mode. Force DDDMP_MODE_BEST."); /* falls through */
  ...
  if (noHeader==0) {
    ...
    for (i=0; i<rootN; i++) {
      ...
      for (j=(int)strlen(tmpString); j<intStringLength+1; j++) {
        strcat (tmpString, " ");
      }
  ...
  *clauseNPtr = clauseN;
  ...
  if (noHeader==0) {
    ...
    for (j=(int)strlen(tmpString); j<2*(intStringLength+1); j++) {
      strcat (tmpString, " ");
    }
